...
Purpose
----------
This code provides functions for training and simulating DNN models.
...
from src.models import DeepCNN, ModelGenerator
...
class TrainingParams(object):
    ...
    def set_model(
        self,
        model_config: ModelGenerator = None,
    ):
        """
        ...
        - model_config(ModelGenerator): ModelGenerator instance, contains the model configuration.
        ...
        Raises
        ------
        Exception: If the model is not defined.
        """
        if isinstance(model_config, ModelGenerator):
    ...
    def load_model(self, loading_path: Path):
        ...
        self.model.load_state_dict(torch.load(
            loading_path, map_location=device))
        return self
...
def train(
    ...
    if plot_curves:
        plot_learning_curve(
            list(range(training_parameters.epochs)
...
def train_model(training_params: TrainingParams, model_name: str, checkpoint_path=None):
    ...
    Raises
    ------
        - Exception: if model type is not defined for training
    """
    ...
    for epoch in range(training_params.epochs):
        ...
        for data in tqdm(training_params.train_dataset):
            ...
            if training_params.model_type.startswith("DeepCNN"):
                ...
                raise Exception(
                    f"train_model: model type {training_params.model_type} is not defined for training")
            ...
            try:
                ...
                print(
                    "train_model: Loss cannot be calculated due linalg error, moving to the next batch.")
            # optimizer update
            optimizer.step()
            # reset gradients
            model.zero_grad()
    ...
    print(
        "Minimal Validation loss: {:4f} at epoch {}".format(
            min_valid_loss, best_epoch)
    )
...
def simulation_summary(
    ...
    if model_type.startswith("DeepCNN"):
        print(f"classifier grid size = {parameters.grid_size}")
...
def get_simulation_filename(
    ...
    return (
        ...
        + f"grid_size={model_config.grid_size}"
    )
