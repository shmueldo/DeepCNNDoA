...
def find_roots_torch(coefficients: torch.Tensor):
    ...
    """
    A = torch.diag(torch.ones(len(coefficients) -
                   2, dtype=coefficients.dtype), -1)
    A[0, :] = -coefficients[1:] / coefficients[0]
...
def get_k_angles(grid_size: float, k: int, prediction: torch.Tensor):
    ...
    """
    limit_angle = (grid_size // 2)
    angles_grid = torch.linspace(
        start=-limit_angle, end=limit_angle, steps=grid_size
    )
    ...
    while doa_prediction.shape[0] < k:
        ...
        doa_prediction = np.insert(
            doa_prediction, 0, np.round(
...
def get_k_peaks(grid_size: int, k: int, prediction: torch.Tensor):
    ...
    peaks, peaks_data = scipy.signal.find_peaks(
        prediction.detach().numpy().flatten(), prominence=0.05, height=0.1
    )
    peaks = peaks[np.argsort(peaks_data["peak_heights"])[::-1]]
    doa_prediction = angles_grid[peaks]
    while doa_prediction.shape[0] < k:
        # print(f"{algorithm}: cant estimate M sources")
        doa_prediction = np.insert(
            doa_prediction, 0, np.round(
...
def gram_diagonal_overload(Kx: torch.Tensor, eps: float, batch_size: int):
    ...
    for iter in range(batch_size):
        ...
        # Diagonal loading
        eps_addition = (
            eps * torch.diag(torch.ones(Kx_garm.shape[0]))).to(device)
